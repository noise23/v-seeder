...
public:
  ...
  void ParseCommandLine(int argc, char **argv) {
    static const char *help = "Version-seeder\n"
...
extern "C" void* ThreadDumper(void*) {
  ...
  do {
        ...
        count++;
    {
      ...
      FILE *f = fopen("vseed.dat.new","w+");
      if (f) {
        {
          CAutoFile cf(f);
          cf << db;
        }
        rename("vseed.dat.new", "vseed.dat");
      }
      FILE *d = fopen("vseed.dump", "w");
...
static const string mainnet_seeds[] = {"node1.version2.org", "node2.version2.org", "node3.version2.org", "node4.version2.org", ""};
static const string testnet_seeds[] = {""};
static const string *seeds = mainnet_seeds;

extern "C" void* ThreadSeeder(void*) {
  if (!fTestNet){
    db.Add(CService("kjy2eqzk4zwi5zd3.onion", 9988), true);
  }
  do {
...
int main(int argc, char **argv) {
  ...
  FILE *f = fopen("vseed.dat","r");
  if (f) {
    printf("Loading vseed.dat...");
    CAutoFile cf(f);
    cf >> db;
    if (opts.fWipeBan)
        db.banned.clear();
